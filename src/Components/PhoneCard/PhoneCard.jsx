...
const PhoneCard = ({phone}) => {
    const {id,image,brand_name,phone_name,} = phone || {}
      ...
      const handleAddButton = () =>{
        ...
        const findFavorite = JSON.parse(localStorage.getItem('favorites'))
        ...
        if(!findFavorite){
            ...
            const exists = findFavorite.find(phone => phone.id === id)
            if(!exists){
                addedFavoriteArray.push(...findFavorite,phone)
                localStorage.setItem('favorites',JSON.stringify(addedFavoriteArray)) 
            }
            else{
                swal("error!", "duplicate product not add", "error");
            }
